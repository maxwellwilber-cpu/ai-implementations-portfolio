...
**Working-with-I section.** Explicit instructions on how to interact: give real feedback on my own copy (not sycophantic agreement), match response to ask (file vs. chat), reference my actual stack (HubSpot, Instantly, Prosp_AI, Airtable) instead of suggesting generic new tools.
...
**Tool-stack grounding.** References my actual tools (HubSpot for CRM, Instantly and Prosp_AI for outbound, Airtable for competitor matrix) and routes work to the right internal owner by function rather than inventing a generic org chart. Recommendations stay inside his real workflow instead of drifting into "install a new tool" territory.
...
**Efficiency (based on my own usage):**
