...
- **[Enterprise Sales Intelligence System](implementations/02-enterprise-sales-intelligence/)** — 615-line source-of-truth consolidating 6 fragmented sales assets via a multi-session AI pipeline with context-transfer documents; designed to serve as the SDR onboarding document as the RevSend team scales.
...
- **[Outbound Sales Agent Skill](implementations/12-outbound-sales-agent-skill/)** — Opinionated Claude skill that front-loads RevSend's product, ICP, competitive battle cards, and messaging principles so every invocation produces RevSend-specific outbound output rather than generic sales advice; compressed per-prospect prep time ~90% (30 min → 3 min) for my own usage.
...
| | |
|---|---|
| **What I Built** | A Claude skill that gives Claude deep, in-context knowledge of RevSend — product differentiators, ICP and buyer personas, competitive battle cards, sales motion, use case playbooks, and messaging principles — so every outbound invocation produces RevSend-specific output instead of generic sales advice. Installed at the RevSend org level. |
| **Technical Details** | Engineered system prompt organized around 4 response modes (Targeting & Research, Messaging & Copy, Pipeline & Organization, Strategy & Planning). Front-loads 6 core product differentiators, 4 buyer personas with pain points, 3 competitors with per-objection responses, 4 use case playbooks with trigger/cadence outlines, and 6 explicit messaging principles that act as filters on any copy output. Distills the sales knowledge developed across implementations #1 (master strategy), #2 (ICPs), #3 (messaging), and #5 (battle cards) into a single invokable context. References Max's actual stack (HubSpot, Instantly, Prosp_AI, Airtable) and teammates by name rather than suggesting generic tools. |
| **Business Result** | Converted RevSend's outbound playbook from static knowledge into an invokable agent. Currently used by 4 SDRs and the 2 co-founders. Any team member can now get ICP-aligned, competitively-positioned, on-brand guidance in a single prompt rather than mentally assembling it from multiple sources. Compressed per-prospect prep time ~90% (30 min → 3 min) for my own usage; team-wide measurement pending as adoption scales. |
| **My Role** | Authored the full system prompt. Made the prompt-engineering decisions that shape output quality — opinionated framing over neutral, role conditioning before content, response-mode calibration, embedded anti-sycophancy instruction, RevSend-specific grounding throughout. Packaged and deployed at the org level. |
